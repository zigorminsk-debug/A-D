...
Исходники уже лежат в этом репозитории (`bp-diary-app/`). После каждого push GitHub Actions собирает APK постоянным ключом `bp-diary-app/keystore/bp-diary.keystore` и кладёт его в артефакт **BPDiary-APK** и в релиз `latest-apk`. Ключ не заменять — иначе обновление не встанет поверх. Правила для агентов: `AGENTS.md`.
